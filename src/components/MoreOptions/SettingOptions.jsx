...
function SettingOptions() {
    ...
    return (
        <ul className=" bg-ig-primary-background w-[400px] z-100 rounded-xl">
            {data.username === user.username && <li className="text-base text-center text-ig-primary-text bg-transparent  px-2 py-3 rounded-t-xl border-0 hover:bg-gray-100 cursor-pointer">
                <button onClick={() => {
                    dispatch(hideModal());
            ...
            {data.username === user.username && <li className="text-sm text-center text-ig-primary-text bg-transparent  px-2 py-3  border-0 hover:bg-gray-100 border-t border-ig-elevated-separator cursor-pointer">
                <button onClick={() => {
                    logOut(authDispatch);
                    dispatch(hideModal())
